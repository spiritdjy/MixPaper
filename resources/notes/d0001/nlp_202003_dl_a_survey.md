...
#### 深度学习时代的问题解答
- [ qiu2015convolutional ]使用卷积神经网络以便以固定长度矢量的形式对Question-Answer句子对进行编码，而与输入句子的长度无关。它们没有使用诸如余弦相关性的距离度量，而是合并了非线性张量层以匹配问题和答案之间的相关性
![](../../images/d0001/07603161221201081612.png)
没有使用诸如余弦相关性的距离度量，而是合并了非线性张量层以匹配问题和答案之间的相关性
![](../../images/d0001/07603531221201105312.png)

- [ kumar2016ask ]引入了情景记忆。在其动态内存网络（DMN）中。通过处理输入序列和问题，DMN形成情景记忆来回答相关问题。如图14所示  ，他们的系统是基于原始的Input-Question-Answer triple进行训练的
![](../../images/d0001/07603341221201133412.png)
![](../../images/d0001/07603561221201135612.png)
输入模块编码原始输入文本到分布式矢量表示; 问题模块将问题编码为其分布式矢量表示形式。情景记忆模块使用注意机制，以便集中在输入模块的特定部分。通过迭代过程，此模块将生成考虑问题以及先前内存的内存矢量表示。的答案模块使用最终存储载体以产生一个答案
DMN是可以潜在地用于各种NLP应用程序（例如分类，问题解答和序列建模）的体系结构之一

- [ xiong2016dynamic ]引入了动态协作网络（DCN），以解决与错误答案相对应的局部最大值；被认为是回答问题的最佳方法之一
#### 
